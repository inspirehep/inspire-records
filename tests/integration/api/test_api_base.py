...
from inspire_records.api.base import InspireQueryBuilder
...
def test_query_builder_returns_not_deleted(base_app, db, create_record):
    ...
    not_deleted_records = InspireQueryBuilder() \
        .not_deleted() \
        .query() \
        .all()
...
def test_query_builder_returns_by_collections(base_app, db, create_record):
    ...
    literature_records = InspireQueryBuilder() \
...
def test_query_builder_returns_no_duplicated_not_deleted_by_collections(base_app, db, create_record):
    record = create_record("lit", data={'deleted': False, '_collections': ['Literature'], 'control_number': 1})
    create_record("lit", data={'deleted': False, '_collections': ['Other'], 'control_number': 2})
    create_record("lit", with_pid=False, data={'deleted': False, '_collections': ['Literature'], 'control_number': 1})
    create_record("lit", data={'deleted': True, '_collections': ['Literature']})

    
    records = InspireQueryBuilder() \
        .by_collections(['Literature']) \
        .not_deleted() \
        .no_duplicates() \
        .query() \
